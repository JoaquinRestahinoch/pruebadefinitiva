...
const tips = [
  {
    ...
    description: "Clear, well-lit photos work best for AI processing",
  },
  {
    icon: SunIcon,
    title: "Good lighting matters",
    description: "Ensure your product is evenly lit without harsh shadows",
  },
  {
    icon: FrameIcon,
    title: "Center your product",
    description: "Position the product in the middle of the frame",
  },
  {
    icon: EraserIcon,
    title: "Simple backgrounds",
    description: "Plain or white backgrounds give the best results",
  },
];

async function uploadProductReal(
  file: File
): Promise<{ ok: boolean; product_id?: string; detail?: string }> {
  ...
  if (!res.ok) {
    return { ok: false, detail: data?.detail || "Failed to upload image" };
  }

  return { ok: true, product_id: data?.product_id };
}
...
export default function UploadPage() {
  ...
  const handleFileSelect = useCallback(async (selectedFile: File) => {
    ...
    reader.onload = async (e) => {
      ...
      const result = await uploadProductReal(selectedFile);
      ...
      if (result.ok && result.product_id) {
        ...
        setUploadState("error");
        setErrorMessage(
          result.detail || "Failed to upload image. Please try again."
        );
      }
    };
      ...
      <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <header className="mb-10 text-center">
          ...
          <p className="mt-2 text-slate-400">
            Add a photo of your product to generate stunning AI backgrounds
          </p>
        </header>
          ...
          <section className="lg:col-span-3">
            <article className="overflow-hidden rounded-xl border border-white/10 bg-slate-900 shadow-[0_1px_0_0_rgba(255,255,255,0.04)]">
              ...
              <div
                ...
                onKeyDown={
                  !preview
                    ? (e) => e.key === "Enter" && handleBrowseClick()
                    : undefined
                }
                className={`relative transition-all duration-200 ${
                  uploadState === "dragging" ? "bg-white/5" : ""
                } ${
                  !preview ? "cursor-pointer hover:bg-white/5" : ""
                }`}
                role={!preview ? "button" : undefined}
                tabIndex={!preview ? 0 : undefined}
                aria-label={!preview ? "Click or drop files here to upload" : undefined}
              >
                <input
                  ref={inputRef}
                  type="file"
                  ...
                  {preview ? (
                    <div className="relative h-full w-full">
                      <img
                        src={preview}
                        alt="Product preview"
                        className="h-full w-full object-contain bg-black/30 p-4"
                      />
                  ...
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-2 rounded-lg bg-white px-4 py-2 text-sm font-semibold text-slate-950 transition-colors hover:bg-white/90 disabled:cursor-not-allowed disabled:opacity-50"
                      >
                        Continue
              ...
              {!file && uploadState !== "error" && (
                <div className="border-t border-white/10 p-6">
                  <button
                    ...
                    className="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-white/15 bg-white/5 px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-white/10"
                  >
                    ...
                    {Array.from({ length: totalSteps }, (_, i) => (
                      <div key={i} className="flex items-center gap-2">
                        ...
                        {i < totalSteps - 1 && (
                          <div
                            className={`h-px w-6 ${
                              i + 1 < currentStep ? "bg-white" : "bg-white/15"
                            }`}
                          />
                        )}
                      </div>
